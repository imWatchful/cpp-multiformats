...
namespace varint
{
    ...
    constexpr std::size_t max_encoded_size() noexcept
    {
        return (std::numeric_limits<T>::digits + 6) / 7;
    }

    template <std::unsigned_integral T>
    constexpr encode_error
    encode(T value, std::span<std::uint8_t> out, std::size_t& bytes_out) noexcept
    {
        constexpr std::size_t spec_max_bytes = 9;
        constexpr std::size_t max_bytes = (max_encoded_size<T>() < spec_max_bytes)
            ? max_encoded_size<T>()
            : spec_max_bytes;
    ...
    template <std::unsigned_integral T>
    constexpr decode_error
    decode(std::span<const std::uint8_t> in, T& value_out, std::size_t& bytes_out) noexcept
    {
        constexpr std::size_t spec_max_bytes = 9;
        constexpr std::size_t max_bytes = (max_encoded_size<T>() < spec_max_bytes)
            ? max_encoded_size<T>()
            : spec_max_bytes;
        constexpr std::size_t limit_bits = (std::numeric_limits<T>::digits < 63)
